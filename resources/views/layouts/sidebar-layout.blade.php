...
<body class="font-sans antialiased bg-[#eef3f8] text-slate-900">
    @php
        ...
        $homeHref = $authUser->getDashboardRoute();
        ...
        <div class="flex flex-col min-h-0 lg:h-full lg:border-r lg:border-slate-200">
            ...
            <nav class="flex-1 overflow-y-auto px-6 pb-4"
                ...
                <div class="space-y-1">
                    ...
                    @if($authUser->hasRole('super_admin') && Route::has('filament.superadmin.pages.dashboard'))
                    <a href="{{ route('filament.superadmin.pages.dashboard') }}" class="{{ $navClass('filament.superadmin.*') }}">
                        <svg class="h-5 w-5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
